  ...
  <UPageHero :ui="{
    ...
    <template #headline>
      <Motion :initial="{
        ...
        <UColorModeAvatar class="size-20 md:size-24 lg:size-28 ring ring-default ring-offset-3 ring-offset-bg"
          :light="global.picture?.light!" :dark="global.picture?.dark!" :alt="global.picture?.alt!" draggable="false"
          loading="lazy" />
      </Motion>
    </template>
      ...
      <Motion :initial="{
        ...
        <div v-if="page.hero.links" class="flex items-center gap-2">
          ...
          <UButton :color="global.available ? 'success' : 'error'" variant="ghost" class="gap-2"
            ...
            :label="global.available ? 'آماده برای پروژه‌های جدید' : 'در حال حاضر در دسترس نیستم'">
            <template #leading>
    ...
    <UMarquee :overlay="isDesktop">
      <Motion v-for="(img, index) in page.hero.images" :key="index" :initial="{
        ...
        <NuxtImg width="234" height="234" format="webp" loading="lazy" draggable="false" v-bind="img"
          class="rounded-lg aspect-square object-cover" :class="index % 2 === 0 ? '-rotate-2' : 'rotate-2'" />
      </Motion>
    </UMarquee>
  </UPageHero>
</template>
